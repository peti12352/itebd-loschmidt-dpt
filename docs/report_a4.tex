...
A general state on $L$ spins requires $2^L$ complex amplitudes: one per
configuration of all spins. This is the actual chain, cut at one point into a
left piece and a right piece, not an abstract relabeling. ``Left'' is every
site to one side of the cut, ``right'' is every site to the other side. The
Schmidt decomposition of Eq.~\eqref{eq:schmidt} below is exactly the statement
that the joint state of these two physical halves can be written as a sum of
$\chi$ pairs of (left state, right state), each weighted by $\lambda_\alpha$.
$\chi$ is not a free parameter, it is the actual number of such pairs needed,
and it equals the rank of the reduced density matrix on either half.

Compression to small $\chi$ is possible because of two physical facts, not
because we choose to throw information away. First, ground states of gapped,
local Hamiltonians obey an area law: the entanglement entropy of any
bipartition is bounded by a constant, independent of how large the two
halves are, instead of growing with the size of the smaller half (a volume
law, which is what a generic random state has). Second, for the
\emph{dynamics} studied here, the Lieb-Robinson bound limits how fast
entanglement can spread after a quench from a low-entanglement initial state:
information and correlations travel at a finite effective speed, so
entanglement entropy across any cut grows at most linearly in time, not
discontinuously to its maximum value. Both facts mean $\chi$ stays small (or
grows only polynomially with time) for the states that actually appear in
this simulation, even though $\chi$ could in principle be as large as
$2^{\min(L,\,L-\ell)}$ for an arbitrary state. This is the entire reason MPS
methods work at all: they are a generic-purpose tool that happens to be very
